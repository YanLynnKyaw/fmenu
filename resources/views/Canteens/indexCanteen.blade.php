...
<body>
    ...
    <div class="sidebar">
        ...
        @if ($schools)
            ...
            <li>
                <a href="{{ route('canteens.index',['school_id' => $school_id]) }}" data-menu="dashboard" class=""><i class='bx bxs-dashboard'></i>{{ $selectedSchool->school_name }}</a>
            </li>
        ...
        <main>
            <div class="header">
                <div class="left">
                    ...
                    @if(isset($selectedSchool))
                        <h1>{{ $selectedSchool->school_name }}</h1>
                    @endif
                    <div>
                ...
                <div class="orders">
                    <div class="header">
                        ...
                        @if(auth()->user()->can('ed_de','view_only')|| auth()->user()->can('view_only'))
                            @if ($schools)
                            <a href="{{ route('canteens.create', ['school_id' => $school_id]) }}" class="newbtn2">Add New Canteen</a>
                            @endif
                        @endif      
                    </div>
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="butt">
                                    <!-- VIEW -->
                                    <a href="{{ route('category.index', $canteen->canteen_id)}}" class="newbtn"><span class="newbtn_top">View</span></a>
                                    @if(auth()->user()->can('ed_de')|| auth()->user()->can('view_only'))
                                        <!-- Edit -->
                                        <a href="{{ route('canteens.edit',$canteen->canteen_id) }}" class="newbtn"><span class="newbtn_top">Edit</span></a>
                                        <!-- DELETE -->
                                        <form class="newbtnd" action="{{ route('canteens.destroy',$canteen->canteen_id) }}" onclick="confirmation(event)" method="post">
                                            @csrf
                                            @method('DELETE')
